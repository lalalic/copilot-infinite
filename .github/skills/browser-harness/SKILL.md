...
description: 'Direct browser control via CDP using browser-harness CLI. Use when: automating browser tasks, posting to social media (YouTube, XHS, Bilibili, WeChat), scraping web pages, filling forms, uploading files, testing web UIs. Connects to the user''s running Chrome — no puppeteer/playwright needed. Install via uv tool install. Auto-saves successful pipelines for zero-token reuse.'
---

# browser-harness

Direct browser control via CDP. For setup and connection, see [install guide](./references/install.md).

For saved automation pipelines, see [pipelines](./references/pipelines.md).

Domain skills (community-contributed per-site playbooks) are off by default. Set `BH_DOMAIN_SKILLS=1` to enable them; see the bottom section.

**If `BH_DOMAIN_SKILLS=1` and the task is site-specific, read every file in the matching domain-skills directory before inventing an approach.**
...
Profiles (cookies-only login state) live in [interaction-skills/profile-sync.md](./project/interaction-skills/profile-sync.md) — covers `list_cloud_profiles()`, the chat-driven "which profile?" pattern, and `sync_local_profile()` for uploading a local Chrome profile.

## Interaction skills

If you start struggling with a specific mechanic while navigating, look in [interaction-skills/](./project/interaction-skills/) for helpers. They cover reusable UI mechanics like dialogs, tabs, dropdowns, iframes, and uploads. The available interaction skills are:
- [connection.md](./project/interaction-skills/connection.md)
- [cookies.md](./project/interaction-skills/cookies.md)
- [cross-origin-iframes.md](./project/interaction-skills/cross-origin-iframes.md)
- [dialogs.md](./project/interaction-skills/dialogs.md)
- [downloads.md](./project/interaction-skills/downloads.md)
- [drag-and-drop.md](./project/interaction-skills/drag-and-drop.md)
- [dropdowns.md](./project/interaction-skills/dropdowns.md)
- [iframes.md](./project/interaction-skills/iframes.md)
- [network-requests.md](./project/interaction-skills/network-requests.md)
- [print-as-pdf.md](./project/interaction-skills/print-as-pdf.md)
- [profile-sync.md](./project/interaction-skills/profile-sync.md)
- [screenshots.md](./project/interaction-skills/screenshots.md)
- [scrolling.md](./project/interaction-skills/scrolling.md)
- [shadow-dom.md](./project/interaction-skills/shadow-dom.md)
- [tabs.md](./project/interaction-skills/tabs.md)
- [uploads.md](./project/interaction-skills/uploads.md)
- [viewport.md](./project/interaction-skills/viewport.md)
...
- If you need framework-specific DOM tricks, check [interaction-skills/](./project/interaction-skills/) first. That is where dropdown, dialog, iframe, shadow DOM, and form-specific guidance belongs.

## Auto-save successful pipelines (MANDATORY)

**You MUST save every successful browser automation as a reusable pipeline script.** This is non-negotiable — the whole point is zero-token reuse on repeat tasks.

After a browser automation sequence succeeds:

1. **Save the script** to `~/.browser-harness/pipelines/<platform>-<action>.py`
   - Naming: `<platform>-<action>.py` (e.g., `yt-post.py`, `xhs-post.py`, `linkedin-connect.py`)
   - Use JSON data file pattern for user text (no f-strings for CJK/special chars):
     ```python
     import json, tempfile
     data = {"title": "标题", "body": "内容"}
     f = tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False)
     json.dump(data, f, ensure_ascii=False); f.close()
     # script reads: CFG = json.load(open("<data_file>"))
     ```
   - Must include `argparse` with `--help`
   - Must detect login state and stop early if not authenticated

2. **Document the input schema** in [pipelines registry](./references/pipelines.md) — every pipeline MUST have a schema section listing:
   - All input fields (title, description, body, tags, etc.)
   - **Constraints**: max character length, allowed formats, required vs optional
   - **Platform limitations**: e.g., YouTube title max 100 chars, XHS title max 20 chars, WeChat body supports HTML
   - Example JSON so upstream pipelines can prepare data in the right shape

3. **Record in the registry table** — add a row with script name, platform, purpose, and status (✅/⚠️)

4. **Record site-specific learnings** — if you discovered selectors, API quirks, timing requirements, or anti-bot patterns, write them to `~/.browser-harness/domain-skills/<site>/` so future runs don't re-discover them

**Before starting a new automation**, check [pipelines](./references/pipelines.md) first — a saved script may already exist. Run it with `--help` to see if it fits the task. Check the schema to prepare data in the correct format.

## Domain skills (opt-in)

Only applies when `BH_DOMAIN_SKILLS=1`. Otherwise ignore — domain-skills directory is dormant and `goto_url` won't surface skill files.
